...
function setup()
	...
	-- add_slider {var="LENGTH_SCALE", value=LENGTH_SCALE, low=0.8, high=
	
	axiom = '^(-pi/2) (A(1),1) Ge(4)'
	rules = {
		'(A(l),1) : true -> (B(l),0) [ ^(pi/4) (A(l*.8),0) ] [ ^(-pi/4) (A(l*.8),0) ]',
		'(B(x),1) : true -> (S(x,0,.1*x),0) (f(.25*l),0)',
		'(S(l,b,w),10) : true -> S(l,b,w)',
		-- 'I(x) : true -> I(GROWTH_RATE*x)'
	}
	...
	rules = {
			...
			'(B(l,w,n,b),1) : n > 0  -> [ \\(PHI * n) (Gsc(w*.9),0) Gs (B(l,w,n,b),0) Ge(4) ] (S(l,b,w),0) (B(l*LENGTH_SCALE,w*.9,n-1,b),0)',
