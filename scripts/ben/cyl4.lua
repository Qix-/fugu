...
function setup()
	node = fg.meshnode(makeMesh(p1,p2,p3,p4))
	fgu:add(node)
end

function makeMesh(a,b,c,d)
	local hold_fast = new_holdfast(a,b,c,d)

	local donatello = fg.turtle()
	donatello:pushState()
...
function update(dt) 
	t = t+dt
	p4 = math.sin(t*math.pi + math.pi/2)
	--p4 = .2 + .8*p4*p4*p4
	--p2 = 0.1 + 0.2*math.abs(math.sin(t*math.pi))
	p1 = 1 + t
	node:setMesh(makeMesh(p1,p2,p3,p4))
end
...
function new_holdfast(length,width,n,meander)
	...
	obj.build = function(self,donatello)
		for i = 1, self.numRoots do
			...
			length = self.meanLength -- randomN(self.meanLength,.2)
			self.roots[i] = new_root(length,self.rootWidth,meander)

			local p = points[i]
			local dir = normalise(p)
			local up = perp(dir)
			-- if (random()<.5) then up=-up end
			donatello:pushState()
