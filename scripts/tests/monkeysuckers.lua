...
function setup()	
	...
	for i=1,10 do
		local el = randomElement(sphereVerts)
...
function new_thing(v,p,s) -- use the global mesh m
	...
	obj.growSucker = function(self,dt)
		...
		function f(u)
			...
			local exp = stepFunction({{.1,fg.random(.3,.4)},{.3,-.01},{.6,-.6},{1.1,-.1}})(u)
			local d = nil
		...
		for k=1,self.numSegments do
			...
			-- Extrude may not work on the monkey mesh
			-- So call it in protected mode
			-- fg.extrude(m,self.vertex,1,dir,len,exp)

			if (not pcall(fg.extrude,m,self.vertex,1,dir,len,exp)) then
				return false
			end
	...
	obj.update = function(self,dt)
		...
		if (not self.hasGrown) then 
			return self:growSucker() 
		else 
			return self:animateSucker()
		end
