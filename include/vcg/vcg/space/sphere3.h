...
template <class T> void Sphere3<T>::Intersect(const Sphere3<T> &s) {
 ...
 template <class T> int Sphere3<T>::CreateFromBox(int n,  const Point3<T> *points) {
   Point3f pmax(-1e100, -1e100, -1e100);
   Point3f pmin(1e100, 1e100, 1e100);
   for(int i = 0; i < n; i++) {
     for(int k = 0; k < 3; k++) {
       if(pmax[k] < points[i][k]) pmax[k] = points[i][k];
       if(pmin[k] > points[i][k]) pmin[k] = points[i][k];
     }
   }
   Center() = (pmin + pmax)/2;
   Radius() = (pmin - pmax).Norm()/2;
   return 0;
 }
