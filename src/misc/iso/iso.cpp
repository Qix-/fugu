...
int main(int argc, char *argv[])
{
	...
	while(running){
		...
		if (!loadingScript){
			double foofoo = .5 + .5*std::sin(time);
			GLfloat col[] = {foofoo,1-foofoo,foofoo,0.f};
			glMaterialfv(GL_FRONT_AND_BACK,GL_AMBIENT_AND_DIFFUSE,col);
			...
			for(int i=0;i<VOXELSIZE;i++){
				...
				for(int j=0;j<VOXELSIZE;j++){
					...
					for(int k=0;k<VOXELSIZE;k++){
						...
						double r2mod = 1.5*r2 + .2*std::sin(4*time)*(float)math::Perlin::Noise(2*di,2*dj,1*dk);
						float p = math::Perlin::Noise(di,dj+2.7*std::sin(time),dk);

						// volume.Val(i,j,k) = r2mod - .5 + .2*math::Perlin::Noise(dk*3+std::sin(time),di*7,dj+time);
						volume.Val(i,j,k) = r2mod - .5 - .3*p; // + .2*std::sin(time)*math::Perlin::Noise(dk*3+std::sin(time),di*7,dj+time);
						*/
					}
				}
			}
