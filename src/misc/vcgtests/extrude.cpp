...
int main(int argc, char *argv[])
{
	...
	*/
	...
	vcg::tri::io::ExporterOBJ_Point3d<MyMesh>::Save(
		mesh,
		oss.str().c_str(),
...
void extrude(MyMesh* m, MyMesh::VertexType*& v, VPUpdateList& vpul, int width, Point3d direction, double length, double expand){
	...
	/*
	Point3d directionD = direction;
	return fg::Extrude<MyMesh>::extrude(m,
	...
	//std::cout << "extruding ... ";
	//boost::progress_timer _progressTimer; // profile this function
	...
	Point3d newCenter = center + direction*length;
	...
	BOOST_FOREACH(VertexPointer p, internalVerts){
		...
		if (p==v){
			...
			// plus the expansion
			Point3d exp = (newCenter-p->P())*-expand;
			p->P() += exp;
		}
	}
