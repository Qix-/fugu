/**
 * Adapted to allow more drawing modes!
 *
 * @author ben
 */
...
#ifndef FG___VCG_GLTRIMESH
#define FG___VCG_GLTRIMESH
...
public:
	...
	enum ColorMode	{CMNone, CMPerMesh, CMPerFace, CMPerManifoldFace, CMPerVert, CMLast};
    ...
    if(tm==TMPerVert && !TMId.empty()) // in the case of per vertex tex coord we assume that we have a SINGLE texture.
    {
		...
		while( (partial)?(fp!=face_pointers.end()):(fi!=m->face.end()))
		{
			...
			if(!f.IsD())
			{
				...
				if(cm == CMPerManifoldFace){
					bool isManifold = true;
					for(int i=0;i<3;i++){
						vcg::face::Pos<typename MESH_TYPE::FaceType> p(&f,f.V(i));
						for(int k=0;k<3;k++){
							if (not p.IsManifold()){
								isManifold = false;
								break;
