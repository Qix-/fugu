...
MorphCrossSec::MorphCrossSec( const vector<spline::PBezInterpDiv > &interps)
{
...
void MorphCrossSec::addSegment( const spline::PBezInterpDiv &source, const spline::PBezInterpDiv &attractor )
{
	...
	// segment splines
	spline::BezInterp<Vec3> sSeg;
	spline::BezInterp<Vec3> aSeg;
...
double MorphCrossSec::getLength( const spline::BezInterp<Vec3> &spline, double t0, double t1 )
{
    return RombergIntegral(8, t0, t1, spline);
}

double MorphCrossSec::getTime( const spline::BezInterp<Vec3> &spline, double length, int iterations, double tolerance )
{
    if (length <= 0.)
    {
        return 0.;
...
double MorphCrossSec::getSpeed( double t, const spline::BezInterp<Vec3> &spline )
{
...
{
    double** rom = spline::Interpolator<double>::allocate2dArray( order, 2 );
    ...
    spline::Interpolator<double>::delete2dArray( rom );
    return result;
}

double MorphCrossSec::getTotalLength( const spline::PBezInterpDiv &spline )
{
	double length = 0.;
...
spline::PBezInterpDiv MorphCrossSec::addPoint( const spline::PBezInterpDiv &spline, int seg, float localT )
{
	vector<Vec3> cp = spline.getControlPoints();
	vector< pair<Vec3,Vec3> > grad = spline.getGradients();
	spline::BezInterp<Vec3> segInterp = spline.getSegmentInterpolator( seg );
	...
	return spline::PBezInterpDiv( cp, grad );
}
